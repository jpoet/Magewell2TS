...
{
    // Evaluates true if the object has a .timestamp member (e.g., raw parsed audio frames)
    template<typename T>
    concept HasTimestamp = requires(T a) { { a.timestamp } -> std::convertible_to<uint64_t>; };

    // Evaluates true if the object is a pointer containing a ->dts member (e.g., raw AVPacket*)
    template<typename T>
    concept HasPointerDts = requires(T a) { { a->dts } -> std::convertible_to<int64_t>; };

    // Evaluates true if the object has a direct .dts member (e.g., your custom Packet wrapper)
    template<typename T>
    concept HasDirectDts = requires(T a) { { a.dts } -> std::convertible_to<int64_t>; };

    // Evaluates true if the object has an .is_marker flag
    template<typename T>
    concept HasMarkerFlag = requires(T a) { { a.is_marker } -> std::convertible_to<bool>; };
...
#if 0
        else if constexpr (FFmpegHelper::HasPointerDts<T>)
        {
            return static_cast<int64_t>(item->dts);
        }
        else if constexpr (FFmpegHelper::HasTimestamp<T>)
        {
            return static_cast<int64_t>(item.timestamp);
        }
#endif
        return 0;
    }
